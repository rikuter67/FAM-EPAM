...
for i in $(seq 1 $num); do
    for data in "${datasets[@]}"; do
        for pred_len in "${predict_lengths[@]}"; do
            ...
            # 実行コマンドの生成と実行
            python run.py --model tsmixer_rev_in --supplement 'frequence' --training train --data $data --seq_len $seq_len --pred_len $pred_len --learning_rate $learning_rate --n_block $n_block --dropout $dropout --ff_dim $ff_dim --seed 0
